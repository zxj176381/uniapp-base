  ...
  <view class="navigation_bar">
    ...
    >
      ...
      <view class="navigation_bar_title">
        ...
        <slot>
          <text :style="textColor + textSize">{{ title }}</text>
        </slot>
      </view>
...
const { windowWidth, menuButtonInfo, statusBarHeight, platform } = useSystemInfo();
...
const safeAreaTop =
  isDevtools || isAndroid
    ? `height: calc(var(--height) + ${statusBarHeight || 0}px); padding-top: ${statusBarHeight || 0}px;`
    : ``;
...
const textSize = computed(() => {
  ...
  } else if (props.title.length > 14) {
    return "font-size: 22rpx";
  } else {

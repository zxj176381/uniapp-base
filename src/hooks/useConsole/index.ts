...
export function useConsole() {
  ...
  const createLogPrefix = (type: LogConfigIF, moduleName: string) => {
    let message = `[${import.meta.env.VITE_APP_NAME}][${type.name}]`;
    if (moduleName) {
      message += `[${moduleName}]`;
    }
    return message;
  ...
  return {
    logInfo: (...rest: any) => _printLog(logTypes.INFO, rest),
    logDebug: (...rest: any) => _printLog(logTypes.DEBUG, rest),
    logWarn: (...rest: any) => _printLog(logTypes.WARN, rest),
    logError: (...rest: any) => _printLog(logTypes.ERROR, rest)
  };
}
